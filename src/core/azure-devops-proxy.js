(() => {
    ...
    window.AzureDevOpsProxy = {
        ...
        getQueryDateFields: (queryId) => {
            var deferred = $.Deferred();
            var webContext = VSS.getWebContext();

            witRestClient.getQuery(webContext.project.id, queryId, 'all', 1).then(function (query) {
                var dateFields = query.columns.filter(column => {
                    return projectFields.filter(field => field.referenceName == column.referenceName && field.type == 2).length > 0;
                });

                deferred.resolve(dateFields);
            });
